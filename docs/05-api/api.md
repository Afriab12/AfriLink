...
1. **`social.profiles.avatar_media_id` now has a real foreign key (commit `a546c34`), but is still unusable safely today.** The `media` schema (assets/variants/uploads) is migrated and verified, and the column is a proper `ON DELETE SET NULL` FK to `media.assets`. What's still missing is everything an API needs to accept this field responsibly: there is no Media API yet to create/verify an asset, so nothing yet validates that a client-supplied UUID corresponds to a real, owned, `ready`, approved asset — the FK alone only guarantees it points at *some* row, not one the caller owns or that passed processing. Accepting it on `PATCH /me/profile` today would still let a client set an arbitrary UUID belonging to someone else's asset. **Decision unchanged: `avatarMediaId` is not an accepted field on any endpoint yet.** Dependency: the Media API (`docs/05-api/media.md`, design-only) must exist first, per that document's own §15 step 5 sequencing.
...
Known defects and gaps in the implemented API that are **not** design decisions, so they do not belong in the table above. T-1 is fixed (ticked below); T-2 and T-3 are open and unscheduled. Each open item needs its own test-first change and approval. (Messaging WebSocket follow-ups F-1 to F-6 are tracked separately in `docs/05-api/messaging-websocket.md` §14.)

- [x] **T-1 — Malformed UUID path parameters returned `500 INTERNAL_ERROR` instead of `422 VALIDATION_FAILED`. Fixed 2026-09-21 (commit `45c9776`).**
  - **Was:** on all **37** routes that take a UUID path parameter (auth sessions, content, messaging, social graph), a malformed id such as `GET /conversations/not-a-uuid` reached Prisma, which cannot cast it to a `uuid` column, so the client got `500 INTERNAL_ERROR` and the server logged a full Prisma error. (The first probe sampled 15 routes; the complete inventory found 37.)
  - **Now:** each returns `422 VALIDATION_FAILED` with `details: [{ "field": "<parameter name>", "reason": "<parameter name> must be a UUID" }]`, for example `postId must be a UUID`. Authentication and CSRF still answer first, the offending value is never echoed back, and any UUID version in either letter case is accepted (the same rule as the body DTOs). A well-formed id that matches nothing behaves exactly as before (`404 RESOURCE_NOT_FOUND`, or `200` on the four idempotent deletes).
  - **How:** one shared `ParseUuidPipe` (`services/api/src/common/pipes`), applied as `@Param('postId', ParseUuidPipe)`. The two Notifications routes validate through their own param DTO with an identical response (see T-3). `GET /profiles/{userIdOrHandle}` is unchanged: a value that is not a UUID is treated as a handle.
  - **Guarded against regression:** `test/path-params.e2e-spec.ts` covers every UUID route, proves a malformed id never reaches the database, and includes a completeness test that reads Nest's route metadata and **fails if a route with a path parameter is added without being covered**. **A new route that takes an id in its path must use `ParseUuidPipe` and be added to that test's route table.**
...
- [ ] **T-3 — Notifications validates its id path parameter with its own DTO instead of the shared `ParseUuidPipe` (optional cleanup).**
  - **State:** `NotificationIdParamDto` (`@IsUUID()`) and the shared pipe produce the identical `422 VALIDATION_FAILED` response (field `id`), so nothing is broken. The only cost is two mechanisms for one rule.
  - **Fix direction:** in `notifications.controller.ts`, replace `@Param() params: NotificationIdParamDto` with `@Param('id', ParseUuidPipe) id: string` and delete the DTO. The existing Notifications tests and `path-params.e2e-spec.ts` should pass unchanged. Small, and needs no design decision.
