...
| `restrict_community_participation` | `profile` (member being restricted, in the context of a given `communityId`) | `{ "communityId": "01J...", "durationSeconds"?: number }` | **Decided (Decisions recorded, item 6):** `communityId` in `details`, resolved server-side against `CommunityMembership` via `(communityId, targetId)`, reusing `CommunityAccessService`'s existing lookup shape; `404`/`422` if no active membership. **Decided (Decisions recorded, item 17/K.1):** the resulting call to `applyMembershipSanction(membershipId, status)` must always pass `status: 'banned'`, never `'removed'` — see below. |

**Community-restriction enforcement value — decided (Decisions recorded, item 17/K.1):** `CommunityMembership.status` has two values that could in principle carry this action's outcome, `'removed'` and `'banned'`, but they are **not interchangeable** — this is an enforcement requirement, not a stylistic convention. `MembershipsService.join()`'s existing re-entry guard blocks a `'banned'` history but not a `'removed'` one, so a member restricted with `status: 'removed'` could simply call `join()` again and undo the restriction immediately. The action-creation flow for `restrict_community_participation` must therefore always resolve to `status: 'banned'` when it calls `applyMembershipSanction` (§7) — `'removed'` remains reserved exclusively for the ordinary, non-moderation staff/owner "kick" outcome (`MembershipsService.remove()`), which stays rejoinable exactly as it is today. No schema change, no new field, and no change to `join()` itself were needed to resolve this — `CommunityMembership.status` remains the sole per-member enforcement state.
...
| Moderation → Communities | `applyMembershipSanction(membershipId, status)` / `liftMembershipSanction(membershipId)` | `restrict_community_participation` — **`status` must always be `'banned'` for this actionType (Decisions recorded, item 17/K.1); `'removed'` is reserved for the ordinary staff/owner "kick" outcome and does not enforce a restriction** (`MembershipsService.join()`'s re-entry guard only blocks `'banned'`) |
...
5. **Content/Messaging/Communities callee-side methods** (§7) — **done and shipped**: `ContentModerationService` (commit `778ef10`), `MessagingModerationService` (commit `ebee91b`), `CommunityModerationService` (commit `11826fc`) — each module's own `applyXModerationStatus`/`resolveXOwnerId`-shaped methods, tests-first, mutation-tested, exported via Nest DI, never routed. **Still outstanding, each its own separate increment:** Messaging's distinct "message removed by moderator" placeholder UX (§4, Decision #15/Q5) — the state write exists, the placeholder rendering does not. Community's `restrict_community_participation` enforcement-value ambiguity is now resolved (K.1, Decisions recorded item 17) — no code change was required, only the calling convention (`status: 'banned'`, never `'removed'`).
...
### Resolved after the callee-side implementations shipped

**17. `restrict_community_participation` enforcement value — K.1, resolved.** Discovered during the Community moderation callee's own review, not part of the original design pass, so recorded here rather than renumbering the list above. `applyMembershipSanction(membershipId, status: 'removed' | 'banned')`'s signature was approved (`docs/04-database/moderation.md` §7/§15 item 2) without specifying which of its two allowed values a moderator-imposed restriction should actually use. Tracing `MembershipsService.join()` showed only `'banned'` blocks re-entry — a `'removed'` row is fully rejoinable, making it a non-enforcing no-op for this actionType. **Owner decision:** the action-creation flow for `restrict_community_participation` must always call `applyMembershipSanction` with `status: 'banned'`; it must never use `'removed'` when the intent is to enforce a restriction. `'removed'` remains exclusively the ordinary, non-moderation staff/owner "kick" outcome. No schema change, no new field, no change to `join()` or `liftMembershipSanction` — `CommunityMembership.status` remains the sole per-member enforcement state, and `liftMembershipSanction` (already shipped, `CommunityModerationService`) already restores either value to `'active'` unconditionally, without re-evaluating `membershipPolicy`.
