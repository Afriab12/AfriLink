...
- **Response:** identical shape to the normal appeal-creation response (`201`, the created `Appeal` row) — from the caller's perspective past this one entry point, everything else about appeals (moderator review, decide, notifications once built) is unchanged.
