...
**A concrete database design now exists: `docs/04-database/media.md`** (2026-09-22, design-only, pending owner review). It resolves the field types, enums vs. validated text, the `content.post_media`/`messaging.message_attachments` attachment-table architecture, the avatar/cover foreign-key strategy, retention, indexing, and security for this schema — at the same level of detail this section's Communities/Messaging/Notifications counterparts had before *those* were implemented. The prose below is the original, pre-design-review sketch that document was written from; it is superseded by `media.md` for anything the two disagree on. No Prisma model, migration, or dependency exists for either.
...
3. Add communities, messaging, notifications, media, moderation, audit, feed, and search projections incrementally. **Done:** communities, messaging and notifications (database layer). **Design reviewed, not yet implemented:** media (`docs/04-database/media.md`). **Remaining:** moderation, audit, feed, search. Messaging required no further retention decision (ADR-003 §5); notifications still need one (open, §24); media has its own smaller set of open product-policy decisions (`media.md` §14).
...
This document's design is fully approved (PRD, ADR-001, ADR-002, ADR-003). Step 2 (Phase 1 Prisma schema and migration) is complete, and step 3 is in progress: the communities, messaging and notifications database layers are implemented — see the Status line at the top of this document. Media has a reviewable design (`docs/04-database/media.md`) but is not yet implemented. Moderation, audit, feed and search have not yet been started in this repository.
