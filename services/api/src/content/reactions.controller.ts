...
export class ReactionsController {
  ...
  async setPostReaction(@CurrentUser() user: { sub: string }, @Param('postId') postId: string, @Body() dto: SetReactionDto) {
  ...
  @HttpCode(200)
  @UseGuards(JwtAuthGuard, CsrfGuard)
  async removePostReaction(@CurrentUser() user: { sub: string }, @Param('postId') postId: string) {
  ...
  @HttpCode(200)
  @UseGuards(JwtAuthGuard, CsrfGuard)
  async removeCommentReaction(@CurrentUser() user: { sub: string }, @Param('commentId') commentId: string) {
